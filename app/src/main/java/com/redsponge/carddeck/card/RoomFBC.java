...
public class RoomFBC implements Iterable<PileFBC> {
    ...
    private List<String> newPileIds;
    ...
    public void updatePileCollections() {
        synchronized (this) {
            if (newPileIds.size() > 0) {
                pileOrderList.addAll(newPileIds);
                newPileIds.clear();
                pushPileOrder();
            }
        }
    ...
    public PileFBC createPile(float x, float y, long chosenTime, CardFBC... cards) {
        ...
        synchronized (this) {
            ...
            newPileIds.add(newPileReference.getKey());
        }
        return fbc;
    }
